...
search_coord <- function(long, lat) {
  ...
  if(!is.numeric(long) || !is.numeric(lat)) stop("Longitude and latitude must be numbers.")

  if(length(long) != length(lat)) stop("Number of longitude and latitude coordinates must be equal.")
  ...
  if(!file.exists(cache_file())){
    ...
    search_df <- tibble::tibble(long = long, lat = lat) |>
      ...
      dplyr::mutate(dist = purrr::map2(.data[["X"]], .data[["Y"]], closest_centoid)) |>
      tidyr::unnest("dist") |>
    ...
    if(max(as.numeric(search_df$dist)) >= 1000) stop("One or more coordinates greater than 1000m from nearest quarter section center. Please check your data.")

    if(max(as.numeric(search_df$dist)) > 600 && max(as.numeric(search_df$dist)) < 1000) warning("One or more coordinates greater than 600m from nearest quarter section center. Please check your data.")
    search_df
  }
}
