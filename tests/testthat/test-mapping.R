...
test_that ("Messages, warnings, and errors show up", {
  expect_error(map_quarter(x = c("SW-6-35-29W1", "NW-7-33-29W1")),
               "Must provide a dataframe")
  expect_error(map_quarter(x = data.frame("Location" =
                                            c("SW-6-35-29W1", "test"))),
               "Dataframe must have a coloumn named legal")
  expect_warning(map_quarter(x =
                               data.frame("legal" = c("SW-6-35-29W1", "test"))),
